...
public class HomeActivity extends AppCompatActivity implements SeekBar.OnSeekBarChangeListener, CustomResponseListener {
    ...
    private void init() {
        ...
        loansList.add(new LoanType("Wedding Loan", R.mipmap.wedding, Constants.Keys.WEDDING));
        loansList.add(new LoanType("Home Loan", R.mipmap.home, Constants.Keys.HOME_LOAN));
        loansList.add(new LoanType("Car Loan", R.mipmap.car, Constants.Keys.CAR));
        loansList.add(new LoanType("Wheeler Loan", R.mipmap.wheelers, Constants.Keys.WHEELERS));
    ...
    private int getLoanLength(float amountReq, float monthluPay) {
        if (monthluPay <= 100) {
            return 1;
        } else {
            try {
    ...
    private int getMonthlyRepayment(int amountReq, int loanLength) {
        try {
